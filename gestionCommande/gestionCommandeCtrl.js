let vm = new Vue({
  ...
  methods: {
    ...
    updateOrderState() {
      ...
      fetch(
        this.constant.BASE_URL_USER +
          this.currentOrder.customerInformation.userkey +
          "/orders/" +
    ...
    exportClientToCsv() {
      vm.usersToExport = [];
      this.$http.get(this.constant.BASE_URL_ORDERS).then(
        (res) => {
     ...
     */
    saveAd() {
      this.$http.put(this.baseUrlAd, JSON.stringify(this.currentAd)).then(
        (res) => {
        ...
        (err) => {
          ...
          swal(
            "",
            "Une erreur interne est survenue, veuillez re essayer ultérieurement",
            "error"
          );
        }
      );
     ...
     */
    getPublicityLink() {
      this.$http.get(this.baseUrlAd).then(
        (res) => {
